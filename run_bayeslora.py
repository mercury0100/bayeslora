#!/usr/bin/env python3
# lora_svd_vs_lora_mirror.py
# Vanilla LoRA vs Variational LoRA (Sparse Variational Dropout / ARD)
# Mirrors Molchanov et al. (2017) training: per-weight KL, ELBO minibatch scaling, β warm-up, paper's pruning rule.
...
class VariationalLoRALinear(nn.Module):
    ...
    def forward(self, x, sample: bool = True):
        ...
        A_log_s2 = torch.clamp(A_log_s2, min=-15.0, max=5.0)
        B_log_s2 = torch.clamp(B_log_s2, min=-15.0, max=5.0)
        ...
        if self.local_reparam:
            ...
            # s = x A^T  -> mean: m_s = x @ A_mu^T ; var: v_s = (x^2) @ diag(Σ_A)^T
            x2 = x.pow(2)
            A_var = A_log_s2.exp()  # [r, d_in]
            m_s = x @ self.A_mu.t()                        # [B, r]
            v_s = x2 @ A_var.t()                           # [B, r]

            # Δy = s B^T  -> mean: m_y = m_s @ B_mu^T
            # Var approx: v_y ≈ v_s @ (B_mu^2)^T + (m_s^2) @ diag(Σ_B)^T
...
def train_once(mode: str, cfg: TrainConfig, train_loader, val_loader, tag: str):
    ...
    N = len(train_loader.dataset)        # train size (for alt scaling if desired)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            if mode == "vlora":
                # Faithful ELBO minibatch scaling: loss = CE + β * reg * (KL_sum / B)
                kl_sum = model.kl()
                kl_term = cfg.kl_scale * (kl_sum / B)
                loss = ce + beta * kl_term
                tot_kl += float((kl_sum / B).item())
            else:
                loss = ce
        ...
        if mode == "vlora":
            ...
            print(
                ...
                f"kl {(tot_kl if B>0 else 0):.4f} | β {beta:.2f} | sample {int(do_sample)} | "
                f"val_acc {val_acc*100:.2f}% | L1 {sp['layer1']} | L2 {sp['layer2']}"
            )
        else:
...
def main():
    ...
    p.add_argument("--kl-scale", type=float, default=0.05,
                   help="KL multiplier (aka reg_factor). Loss = CE + β * kl_scale * (sum(KL)/#batches)")
